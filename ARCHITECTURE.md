...
| 방어선 | 무엇을 한다 | 막아주는 사고 |
|---|---|---|
| **① 활성 탭 강제** | 발송 시작 전 카톡 메인 창이 발송 대상 탭(친구 또는 채팅) 으로 활성화돼있는지 검증. 직전에 열려 있던 임의의 채팅창 컨텍스트를 명시적으로 비움 | 직전 사용자 액션이 만든 잔여 포커스가 엉뚱한 입력창에 텍스트 떨구는 사고 |
...
##### ⭐ 검색 탭 일반화 (2026-05-23 본인 PC 실측 반영)

원래 방어선 1은 "친구 탭 강제" 였으나, 본인 환경 실측에서 **"나와의 채팅" 및 본인 자신은 친구 탭 검색에 안 잡히고 채팅 탭 검색에서만 잡힘** 을 발견. 다음으로 일반화:

- `EnsureFriendsTabStep.expected_tab` 옵션: `"friends"` (기본, 단톡 혼재 방지) / `"chats"` (자기 자신 등 친구 탭 미등록 대상) / `"any"` (둘 중 하나만 활성이면 OK)
- 검색 Edit / 목록 컨트롤 찾기는 활성 탭 자동 감지 (`window_spec.find_search_edit_in_active_tab`, `window_spec.find_active_list_control`)

채팅 탭 모드에서 단톡방이 검색 결과에 섞이는 R7 위험은 방어선 3 (제목 완전일치) 가 완전 차단함 — 따라서 검색 탭이 어디든 안전.

##### ⭐ 별도 창 오픈 방법 (open_method)

본인 환경 실측 결과 **Alt+Enter 가 별도 창을 열지 않음** — 카톡 PC 버전·환경에 따라 다름:

- `"alt_enter"`: `Enter + Alt+Enter` (kakao-sender v2 기본)
- `"enter_only"`: `Enter` 한 번
- `"double_click"`: 친구/채팅 목록 컨트롤 첫 항목에 win32 `WM_LBUTTONDBLCLK` 전송 (마우스 미사용, 본인 환경에서 동작 확인)
